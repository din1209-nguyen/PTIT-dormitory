...
### UML / ERD có thuộc tính

Sơ đồ dưới đây dùng cú pháp Mermaid `erDiagram`: mỗi entity hiển thị như một bảng, gồm kiểu dữ liệu, tên thuộc tính và ký hiệu khóa (`PK`, `FK`, `UK`). Đây là bản schema rút gọn nhưng đủ các collection nghiệp vụ chính; bản đầy đủ hơn nằm trong [`docs/data-model.dbml`](docs/data-model.dbml).
...
### Bảng schema triển khai

| Collection | Thuộc tính chính | Khóa / index quan trọng |
|---|---|---|
| `users` | `username`, `email`, `passwordHash`, `role`, `status`, `tokenVersion`, `lastLoginAt` | `username` UK, `email` UK, index `role`, `status` |
| `permissions` | `code`, `description` | `code` UK |
| `rolepermissions` | `role`, `permissionId` | unique `{ role, permissionId }`, FK `permissionId -> permissions` |
| `refreshtokens` | `userId`, `tokenHash`, `expiresAt`, `revokedAt`, `ipAddress`, `userAgent` | `tokenHash` UK, TTL `expiresAt`, FK `userId -> users` |
| `passwordresettokens` | `userId`, `tokenHash`, `otpHash`, `expiresAt`, `usedAt` | `tokenHash` UK, TTL `expiresAt`, FK `userId -> users` |
| `students` | `userId`, `studentCode`, `fullName`, `gender`, `email`, `phone`, `className`, `major`, `department`, `academicYear`, `isFreshman`, `residenceType` | `studentCode` UK, sparse UK `userId`, index học vụ/trạng thái |
| `semesters` | `name`, `term`, `academicYear`, `startDate`, `endDate`, `status`, `createdBy` | unique `{ term, academicYear }`, index `status`, `startDate/endDate` |
| `residencerecords` | `studentId`, `semesterId`, `registeredAt`, `startDate`, `endDate`, `status` | unique `{ studentId, semesterId }`, FK `studentId`, `semesterId` |
| `buildings` | `name`, `description`, `isActive` | `name` UK |
| `floors` | `buildingId`, `floorNumber`, `description` | unique `{ buildingId, floorNumber }` |
| `rooms` | `floorId`, `roomNumber`, `capacity`, `genderType`, `status`, `isFreshmanPriority` | unique `{ floorId, roomNumber }`, index `genderType`, `status` |
| `beds` | `roomId`, `bedNumber`, `status` | unique `{ roomId, bedNumber }`, index `status` |
| `roomassignments` | `residenceRecordId`, `studentId`, `semesterId`, `roomId`, `bedId`, `assignedBy`, `assignedAt`, `status`, `studentSnapshot`, `roomSnapshot` | partial unique active `{ studentId, semesterId }`, `{ bedId, semesterId }` |
| `regulations` | `title`, `content`, `status`, `createdBy`, `publishedBy`, `publishedAt` | index `status` |
| `notifications` | `title`, `content`, `scope`, `type`, `createdBy`, `createdAt` | index `scope`, `type`, `createdAt` |
| `notificationreceivers` | `notificationId`, `studentId`, `isRead`, `readAt` | unique `{ notificationId, studentId }`, index `{ studentId, isRead }` |
| `studentrequests` | `studentId`, `type`, `title`, `content`, `status`, `managerNote`, `processedBy`, `processedAt` | index `studentId`, `type`, `status`, `createdAt` |
| `violations` | `studentId`, `semesterId`, `description`, `penalty`, `violationDate`, `status`, `createdBy` | index `studentId`, `semesterId`, `status`, `violationDate` |
| `utilityusages` | `roomId`, `month`, `year`, `oldElectricity`, `newElectricity`, `oldWater`, `newWater`, `recordedBy`, `recordedAt` | unique `{ roomId, month, year }` |
| `utilitybills` | `roomId`, `semesterId`, `usageId`, `month`, `year`, `electricityUsage`, `waterUsage`, `electricityCost`, `waterCost`, `vatAmount`, `totalCost`, `status`, `dueDate`, `paymentDate` | sparse UK `usageId`, index `{ roomId, month, year }`, `status`, `dueDate` |
| `utilitybillmembers` | `billId`, `studentId`, `amountShare`, `status`, `paidAt` | unique `{ billId, studentId }`, index `{ studentId, status }` |
| `payments` | `billId`, `studentId`, `method`, `amount`, `status`, `vnpTxnRef`, `vnpTransactionNo`, `vnpResponseCode`, `vnpRawData`, `cashConfirmedBy`, `paidAt` | sparse UK `vnpTxnRef`, index `billId`, `studentId`, `status` |
| `systemconfigs` | `configKey`, `configValue`, `valueType`, `description`, `updatedBy` | `configKey` UK |
| `electricpricetiers` | `tierOrder`, `fromKwh`, `toKwh`, `unitPrice`, `isActive` | `tierOrder` UK, index `isActive` |
| `importbatches` | `semesterId`, `fileName`, `totalRows`, `successRows`, `failedRows`, `status`, `errorMessage`, `importedBy`, `finishedAt` | index `semesterId`, `status`, `createdAt` |
| `importrowerrors` | `importBatchId`, `rowNumber`, `fieldName`, `errorMessage`, `rawData` | index `importBatchId`, `rowNumber` |
| `emaillogs` | `recipientEmail`, `subject`, `content`, `status`, `errorMessage`, `retryCount`, `sentAt` | index `recipientEmail`, `status`, `createdAt` |
| `activitylogs` | `userId`, `action`, `entityName`, `entityId`, `description`, `oldValue`, `newValue`, `ipAddress`, `createdAt` | index `userId`, `action`, `entityName`, `createdAt` |
